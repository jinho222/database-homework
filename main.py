...
def registration(CONNECTION: str, course_id: int, student_id: str) -> Union[list, None]:
    with psycopg.connect(CONNECTION) as conn:
        ...
        query_insert = sql.SQL(
            ...
            values (%s, %s)
        """
        ).format(CR=sql.Identifier(SCHEMA, CR))
...
# problem 4
def withdrawal_registration(
    CONNECTION: str, course_id: int, student_id: str
) -> Union[list, None]:
    with psycopg.connect(CONNECTION) as conn:
        # check
        if not check_course_exists(conn, course_id):
...
# problem 5
def modify_lectureroom(
    CONNECTION: str, course_id: int, buildno: str, roomno: str
) -> Union[list, None]:
    with psycopg.connect(CONNECTION) as conn:
        # check
        if not check_course_exists(conn, course_id):
        ...
        if not check_lecture_room_exists(conn, buildno, roomno):
            print(
                f"“Not Exist lecture room with BUILD NO: {buildno} / ROOM NO: {roomno}"
            )
            return None
...
def check_course_exists(conn, course_id):
    query = sql.SQL(
        ...
        where {COURSE_ID} = %s
    """
    ).format(C=sql.Identifier(SCHEMA, C), COURSE_ID=sql.Identifier("COURSE_ID"))
...
def check_student_exists(conn, student_id):
    query = sql.SQL(
        ...
        where {STUDENT_ID} = %s
    """
    ).format(S=sql.Identifier(SCHEMA, S), STUDENT_ID=sql.Identifier("STUDENT_ID"))
